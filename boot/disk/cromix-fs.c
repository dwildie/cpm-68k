...
extern int readDriveSectors(long driveId, long lba, unsigned char *buffer, long count);
...
static int read_fs_block(unsigned int block_number, unsigned char *buffer)
{
  ...
  return readDriveSectors(geometry.drive_id, b, buffer, 1);
}

static int read_geometry(int drive_id, int offset, int verbose)
{
//  printf("readDriveSectors 0x%x\r\n", offset);
  readDriveSectors(drive_id, offset, block, 1);
